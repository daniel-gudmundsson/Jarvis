...
"""
This agent uses one hot encoding for the board
"""
import numpy as np
import Backgammon
import torch
...
#        self.Z_b2 = torch.zeros(self.b2.size(), device=self.device, dtype=torch.float)
    ...
    def forward(self, xnew, xold):
        ...
        x_prime = Variable(torch.tensor(xnew, dtype=torch.float, device=self.device)).view(832, 1)
...
#        x2_sigmoid=x2.sigmoid()
        ...
        x_prime = Variable(torch.tensor(xold, dtype=torch.float, device=self.device)).view(832, 1)
...
#        self.oldtarget=0
    ...
    def forward(self, X):
        ...
        for x in X:
            ###First step is similar to the critic forward propogation
            x_prime = Variable(torch.tensor(x, dtype=torch.float, device=self.device)).view(832, 1)
            
                ###First layer
            x1=torch.mm(self.w1, x_prime) + self.b1
            #x1_sigmoid=x1.sigmoid()
 #           x1_soft=x1.softmax(dim=0)
            ###Second layer (start with only one for now)
    #        x2=torch.mm(self.w2, x1_sigmoid) + self.b2
    #        x2_sigmoid=x2.sigmoid()
            ###Final step
            #y=torch.mm(self.theta, x1_sigmoid) + self.B
            y=torch.mm(self.theta, x1) + self.B
            values[c]=y
            c+=1
...
def action(net, board_copy,dice,player,i):
    ...
    ### Critic feedforward
    target, oldtarget =net.critic.forward(newBoardFeatures, oneHot(board_copy))#(newBoardFeatures,getFeatures(board_copy,player) )
