...
#import agent_share as agent
#import agent_final as agent
import agent_ts as agent
#import agent_share_2 as agent
...
def play_a_game(commentary = False, net=None, random=False, learn=True):
    ...
    while not game_over(board) and not check_for_error(board):
        ...
        for i in range(1+int(dice[0] == dice[1])):
            ...
            if(game_over(board) & learn):
                if player==1:
                    net.gameOverUpdate(1)
                else:
                    net.gameOverUpdate(0)
